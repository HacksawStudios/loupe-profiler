...
class Profiler {
	...
	public static function dumpMark(mark: Mark, traceEvents: Array<TraceEvent>) {
		traceEvents.push({ name: mark.name, cat: cat, ph: "B", pid: pid, tid: tid, ts: Std.int(mark.timestampBegin) });

		for (child in mark.children) {
			dumpMark(child, traceEvents);
		}

		traceEvents.push({ name: mark.name, cat: cat, ph: "E", pid: pid, tid: tid, ts: Std.int(mark.timestampEnd) });
	}
